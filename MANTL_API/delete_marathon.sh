...
if [ "$1" = "" ]; then
	echo "Argument missing.  Requires APP ID"
	exit
fi
...
APP_ID=$1
MANTL_MARATON="$MANTL_PREFIX://$MANTL_SERVER:$MANTL_SERVER_PORT/marathon/v2/apps/$APP_ID"
...
curl -k -X DELETE  --cookie "amlbcookie=01; iPlanetDirectoryPro=$USER_AM_TOKEN"  -H "Content-type: application/json" $MANTL_MARATON
